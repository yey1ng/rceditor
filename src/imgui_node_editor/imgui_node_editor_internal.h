...
# include <base/public/crude_json.h>
# include <vector>
# include <string>
...
struct Node final: Object
{
    ...
    std::shared_ptr<rceditor::NodeInfo> m_NodeInfo{};
...
struct LinkSettings
{
    ...
    LinkSettings(LinkId id,PinId Start, PinId End)
        : m_ID(id)
        , m_StartPinID(Start)
        , m_EndPinID(End)
        , m_WasUsed(false)
    ...
    static bool Parse(const std::string& string, LinkSettings& settings);
...
struct Settings
{
    ...
    LinkSettings* FindLink(LinkId id);
    LinkSettings* AddLink(LinkId id);
...
struct EditorContext
{
    ...
    void RemoveLinkSetingsById(LinkId id);
    ...
    void SaveData(rceditor::NodeManager* i_pNodeManager);
    void LoadData(rceditor::NodeManager* i_pNodeManager);
private:
