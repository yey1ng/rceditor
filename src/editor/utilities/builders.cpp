		...
		if ((HeaderMax.x > HeaderMin.x) && (HeaderMax.y > HeaderMin.y) && HeaderTextureId)
		{
			...
			if (NormalMin.y != NormalMax.y)
			{
				drawList->AddLine(
					...
					ImVec2(TemplateMax.x + (8 - halfBorderWidth), TemplateMax.y + 2.5f),
...
void util::BlueprintNodeBuilder::TemplatePin()
{
	SetStage(Stage::Template);
}

void util::BlueprintNodeBuilder::EndTemplatePin()
{
	SetStage(Stage::Content);
	ImGui::EndHorizontal();

	TemplateMin = ImGui::GetItemRectMin();
	TemplateMax = ImGui::GetItemRectMax();
}

void util::BlueprintNodeBuilder::NormalPin()
{
	SetStage(Stage::Normal);
}

void util::BlueprintNodeBuilder::EndNormalPin()
{
	SetStage(Stage::Content);
	ImGui::EndHorizontal();

	NormalMin = ImGui::GetItemRectMin();
	NormalMax = ImGui::GetItemRectMax();
}
...
bool util::BlueprintNodeBuilder::SetStage(Stage stage)
{
	...
	case Stage::Template:
		ImGui::BeginHorizontal("Template");
		ImGui::Spring(0, 0);
		break;

	case Stage::Normal:
		ImGui::BeginHorizontal("Normal");
		ImGui::Spring(0, 0);
		HasNormalPin = true;
		break;
